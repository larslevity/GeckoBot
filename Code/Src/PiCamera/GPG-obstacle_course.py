...
if __name__ == '__main__':
    ...
    xref = [np.nan, np.nan]
    positions_ = [[0, 70, 70, 0, 0], [0, 0, 70, 70, 120]]

    scale = 112./900
    positions = [
            [400+int(y/scale) for y in positions_[1]],
            [850-int(x/scale) for x in positions_[0]]]

    IMGprocessing.draw_positions(frame, positions, xref, thick=0,
                                 col=(255, 0, 0))
...
#    print('deps:\t', round(eps_opt - eps, 2))
    ...
    cv2.imwrite('GPG-obstacle_course.jpg', frame)
